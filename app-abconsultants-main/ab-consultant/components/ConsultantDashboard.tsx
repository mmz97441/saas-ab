...
const PANEL_CONFIG: Record<Exclude<PanelType, null>, { title: string; color: string; border: string }> = {
    ...
    FRAICHEUR:  { title: 'Dossiers en retard', color: 'bg-amber-50', border: 'border-amber-200' },
            ...
            <tr key={item.client.id} onClick={() => onSelectClient(item.client)} className="hover:bg-white/80 transition-colors cursor-pointer group">
                ...
                <td className="p-2.5 pl-4">
                    <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-full bg-white text-slate-600 flex items-center justify-center font-bold border border-slate-200 text-xs shrink-0">
                        ...
                        <div className="min-w-0">
                            ...
                            <div className="text-xs text-slate-400 truncate max-w-[160px]">{item.client.managerName}</div>
                        </div>
                    </div>
                </td>
                ...
                {(panel === 'CA' || panel === 'SANTE') && (
                    <td className="p-2.5 text-right">
                        ...
                        {panel === 'CA' && portfolioKpis.totalCA > 0 && (
                            <div className="text-xs text-slate-400">{((item.ytdRevenue / portfolioKpis.totalCA) * 100).toFixed(1)}% du total</div>
                        )}
                    </td>
                )}
                ...
                {(panel === 'SANTE') && (
                    <td className="p-2.5">
                        <div className="flex flex-wrap gap-1">
                            {item.treasuryAlert && <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-red-100 text-red-700">Tréso &lt; 0</span>}
                            {!item.dataFresh && <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-amber-100 text-amber-700">Données</span>}
                            {item.ytdObjective > 0 && item.objPerformance < 85 && <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-amber-100 text-amber-700">Obj &lt; 85%</span>}
                        </div>
                    </td>
                )}
                ...
                <td className="p-2.5 text-right pr-4">
                    <span className="text-xs font-bold text-brand-500 opacity-0 group-hover:opacity-100 transition inline-flex items-center gap-0.5">
            ...
            {items.length > 0 ? (
                <div className="overflow-x-auto max-h-[320px] overflow-y-auto">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="text-xs font-bold text-slate-400 uppercase border-b border-white/40">
                                {renderHeader()}
                            </tr>
                        </thead>
...
const ConsultantDashboard: React.FC<ConsultantDashboardProps> = ({ clients, onSelectClient, onNavigateToMessages }) => {
    ...
    if (summaries.length === 0) return (
        <div className="flex flex-col items-center justify-center h-[60vh] px-6 text-center animate-in fade-in duration-500">
            ...
            <div className="flex items-center gap-2 text-sm text-brand-700 bg-brand-50 border border-brand-200 rounded-lg px-4 py-2.5">
                <ChevronLeft className="w-4 h-4" />
                <span>Ouvrez <strong>Portefeuille Clients</strong> dans la barre latérale</span>
            </div>
        </div>
    );
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        className="text-xs font-bold text-brand-600 bg-brand-50 border border-brand-200 px-2.5 py-1.5 rounded-lg hover:bg-brand-100 transition"
                        title="Revenir à M-1"
                    >
                        M-1
                    </button>
            ...
            <div className="text-xs text-slate-500 bg-slate-50 rounded-lg px-4 py-2 border border-slate-100 flex items-center justify-between">
                ...
                {(selectedYear !== defaultYear || selectedMonthIdx !== defaultMonthIdx) && (
                    <span className="text-xs text-amber-600 font-bold bg-amber-50 px-2 py-0.5 rounded-full border border-amber-200">Période personnalisée</span>
                )}
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                ...
                <button onClick={() => togglePanel('CA')} className={kpiCardClass('CA', 'p-4 rounded-xl border bg-white border-brand-100 shadow-sm')}>
                    <div className="flex justify-between items-start mb-2">
                        ...
                        {portfolioKpis.totalObj > 0 && (
                            <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${getPerfColor(portfolioKpis.objPerformance).bg} ${getPerfColor(portfolioKpis.objPerformance).text}`}>
                ...
                <button onClick={() => togglePanel('OBJECTIF')} className={kpiCardClass('OBJECTIF', 'p-4 rounded-xl border bg-white border-brand-100 shadow-sm')}>
                    ...
                    {portfolioKpis.clientsWithObj > 0 && (
                        <div className="mt-1.5">
                            ...
                            <p className={`text-xs font-bold mt-0.5 ${getPerfColor(portfolioKpis.onTargetRate).text}`}>{portfolioKpis.onTargetRate.toFixed(0)}% du portefeuille</p>
                        </div>
                    )}
                </button>
                ...
                >
                    ...
                    {portfolioKpis.staleCount > 0 && (
                        <p className="text-xs font-bold text-amber-600 mt-1">{portfolioKpis.staleCount} dossier{portfolioKpis.staleCount > 1 ? 's' : ''} en retard</p>
                    )}
                </button>
                        ...
                        <div key={config.key} className={`rounded-xl border overflow-hidden shadow-sm ${config.headerClass.split(' ').filter(c => c.startsWith('border-')).join(' ')}`}>
                            ...
                            >
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-xs font-normal opacity-70">({items.length} dossier{items.length > 1 ? 's' : ''})</span>
                                </div>
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <div className="relative shrink-0 mt-0.5">
                                                            <div className="w-9 h-9 rounded-lg bg-slate-100 text-slate-600 flex items-center justify-center font-bold border border-slate-200 text-xs">
                                                                {item.client.companyName.substring(0, 2).toUpperCase()}
                                                            </div>
                                                        ...
                                                        <div className="min-w-0 flex-1">
                                                            ...
                                                            <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                                                                <span className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-xs font-bold ${badge.class}`}>
                                                                ...
                                                                {item.lastRecord && (
                                                                    <span className={`font-mono text-xs font-bold ${item.treasuryAlert ? 'text-red-600' : 'text-emerald-600'}`}>
                                                                        {fmtEur(item.lastRecord.cashFlow.treasury)}
                                                                    </span>
                                                                )}
                                                            </div>
                                                        </div>
                                                        <span className="text-xs font-bold text-brand-500 opacity-0 group-hover:opacity-100 transition shrink-0 mt-0.5">
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex items-center gap-3 min-w-0 flex-1">
                                                            <div className="relative shrink-0">
                                                                <div className="w-8 h-8 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center font-bold border border-slate-200 text-xs">
                                                                    {item.client.companyName.substring(0, 2).toUpperCase()}
                                                                </div>
                                                                {item.client.hasUnreadMessages && (
                                                            ...
                                                            <div className="min-w-0 flex-1">
                                                                <div className="font-bold text-slate-800 text-xs truncate">{item.client.companyName}</div>
                                                                <div className="text-xs text-slate-400 truncate">
                                                                    {item.client.sector && <span className="bg-slate-100 px-1 py-0.5 rounded text-xs mr-1">{item.client.sector}</span>}
                                                                    {item.client.managerName}
                                                        ...
                                                        <div className="flex items-center gap-4 shrink-0">
                                                            {/* CA YTD mini */}
                                                            {item.ytdRevenue > 0 && (
                                                                <span className="hidden lg:inline-block font-mono text-xs font-bold text-slate-400">
                                                                    CA {fmtEur(item.ytdRevenue)}
                                                                </span>
                                                            )}
                                                            ...
                                                            {/* Statut dossier */}
                                                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold ${badge.class}`}>
                                                                {badge.icon} {badge.label}
                                                            </span>

                                                            {/* Trésorerie mini */}
                                                            {item.lastRecord && (
                                                                <span className={`hidden md:inline-block font-mono text-xs font-bold ${item.treasuryAlert ? 'text-red-600' : 'text-emerald-600'}`}>
                                                                    {fmtEur(item.lastRecord.cashFlow.treasury)}
                                                                </span>
                                                            )}
