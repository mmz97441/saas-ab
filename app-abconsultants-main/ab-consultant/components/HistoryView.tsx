                                    ...
                                    <tr key={record.id} className={`border-b border-slate-100 hover:bg-slate-50/50 transition-colors group ${selectedIds.has(record.id) ? 'bg-brand-50/50' : ''}`}>
                                        ...
                                        <td className="p-4 text-center">
                                            <div className="flex flex-col items-center gap-1">
                                                ...
                                                {/* WORKFLOW PIPELINE (consultant) */}
                                                {userRole === 'ab_consultant' && (
                                                    <div className="flex items-center gap-0.5 mt-1">
                                                        <div className={`h-1 w-4 rounded-l-full ${record.isSubmitted ? 'bg-amber-400' : 'bg-slate-200'}`} title="Soumis" />
                                                        <div className={`h-1 w-4 ${record.isValidated ? 'bg-emerald-400' : 'bg-slate-200'}`} title="Validé" />
                                                        <div className={`h-1 w-4 rounded-r-full ${record.isPublished ? 'bg-blue-400' : 'bg-slate-200'}`} title="Publié" />
                                                    </div>
                                                )}
